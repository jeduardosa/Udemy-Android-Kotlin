...
class TaskListViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val taskRespository = TaskRepository(application.applicationContext)
    ...
    fun list(filter: Int) {
        ...
        val listener = object : APIListener<List<TaskModel>> {
            ...
            when (filter){
                TaskConstants.FILTER.ALL -> taskRespository.list(listener)
                TaskConstants.FILTER.NEXT -> taskRespository.listNext(listener)
                else -> taskRespository.listOverdue(listener)
            }
        }

    fun delete(id: Int) {
        taskRespository.delete(id, object : APIListener<Boolean> {
    ...
    fun status(id: Int, complete: Boolean) {
        ...
        if (complete) {
            taskRespository.complete(id, listener)
        } else {
            taskRespository.undo(id, listener)
        }
    }
